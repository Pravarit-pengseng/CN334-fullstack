...
app = FastAPI(title="Appiontment API")
...
async def read_root():
    ...
    <html>
        ...
        <body>
            ...
            <p>pravairt pengseng 6710742609 </p>
        </body>
    </html>
    """
...
async def create_patient(
    record: schemas.PatientRecordCreate, db: Session = Depends(get_db)
):
...
async def update_patient(
    record_id: int, record: schemas.PatientRecordCreate, db: Session = Depends(get_db)
):
...
async def create_appointment(
    form_data: schemas.AppointmentForm, db: Session = Depends(get_db)
):
...
async def edit_appointment(
    ...
    updated_data = crud.update_appointment(
        db=db, appointment_id=appointment_id, appointment=form_data
    )
